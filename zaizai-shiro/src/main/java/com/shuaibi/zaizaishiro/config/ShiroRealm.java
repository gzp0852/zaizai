...
import com.shuaibi.zaizaishiro.model.User;
...
public class ShiroRealm extends AuthorizingRealm {
	...
	protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken token)
		 ...
		 * 注:用于存放用户信息的模型,必须能后实例化。即:必须实现Serializable接口 注:
		...
		/*
		 * 7.credentials: 凭证(一般都是密码). credentials本应该是查询出来的;这里为了快速测试,我们直接写
		 */
		...
		/*
		 * 9. 盐值. 注:如果多个用户的密码一样，那么一般情况下加密结果也一样; 注:通过使用不同的盐值来确保即便密码都一样,加密结果也会不一样 注:盐值
		 * 最好保证其唯一性。 注:由于一般情况下,用户名是唯一的，所以我们一般使用用户名来计算盐值
		 */
		ByteSource credentialsSalt = ByteSource.Util.bytes(username);

		/*
		 * 实例化对象. 注意:如果不加密,那么就是直接比对的明文
		 * 注意:SimpleAuthenticationInfo加密,是指:将用户登录时输入的密码(盐值)加密后,与数据库取出来的密码进行比对
		 * 所以,这里的加密并不是对从数据库取出来的credentials进行加密!从数据库取出来的credentials应该是 之前录入数据库时已经加密好了的.
		 */
	...
	protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection principals) {
		 ...
		 * 注:默认的策略为AtLeastOneSuccessfulStrategy,那么这里只能获取 到对应的通过了身份认证的Realm中的principal
		 * 注:就算有多个Realm,一般而言这些Realm中的principal都应该是一样的,所以 我们读取信息时，一般拿其中的某一个进行读取就行
		 */
		Object principal = principals.getPrimaryPrincipal();

		/*
		 * 2.从配置applicationContext.xml中可知:我们写的ShiroReaml是第一个,
		 * 而ShiroReaml中的principal,我们传的是User对象 所以这里获取到的principal即为该User对象,并获取到对应其角色信息
		 */
